...
class Tscombine(object):
    ...
    def measure_dispersion(self, periods, alpha, shift_len, deltav, period,
                           rminv=2, rmaxv=7):
        """
        ...
        # Isolation
        isotr1, weight1 = movement_windows_construction(tr1, periods, group_arr1,
                                                        reftime, period)
        isotr2, weight2 = movement_windows_construction(tr2, periods, group_arr2,
                                                        reftime, period)
        
        #########################
        # Kaiser filter and cross
        # correlation calculation
        #########################
        
        
        # cross-correlation param. defination
        xcorr = np.zeros(shape=(len(periods), int((rmaxv-rminv)/deltav)))
        for iperiod, T0 in enumerate(periods):

            logger.debug("Processing {} s".format(T0))
            # kaiser filter
            signal1 = isotr1[iperiod,:][weight1[iperiod,:] != 0]
            signal2 = isotr2[iperiod,:][weight2[iperiod,:] != 0]
            filwave1 = kaiser_windows_filter(tr1, signal1, T0)
            filwave2 = kaiser_windows_filter(tr2, signal2, T0)
            
            # Construct velocity-period map
            veloscale, xcorr[iperiod,:] = VeloPeriodMatrix(tr1, tr2, filwave1,
                                                           filwave2, T0,
                                                           deltav= deltav, 
                                                           shift_len=shift_len)
    ...
    def extraction_of_phasevelo(self, periods):
        ...
        refpoint = self.plot_xcorr(periods)
        if not refpoint:
    ...
    def plot_xcorr(self, periods, dispersion=None):
        """
...
def VeloPeriodMatrix(tr1, tr2, wave1, wave2, T0, rmaxv=7, rminv=2, deltav=0.005,
                     shift_len=500):
    """
    ...
    correlation = correlate(wave1, wave2, shift=npoints)
    ...
    if len(correlation) == len(veloscale): 
        ...
        maskarray = (veloscale < rmaxv) * (veloscale > rminv)
    #splvector = np.abs(correlation[maskarray])
    ...
    xcorr = interpolate.splev(intersveloscale, tck, der=0)

    return intersveloscale, xcorr
...
def kaiser_windows_filter(tr, signal, T0, window=('kaiser', 9.0),
    ...
    # Configuration
    ntaps = 2 ** math.ceil(math.log(signal.shape[0], 2))
    
    HalfFilterNum = np.round(ntaps / 2)

    # extent signal with adding zeros ahead or tail the signal
    filtwave = np.zeros(int(HalfFilterNum + len(signal)))
    WinWave = np.append(signal, np.zeros(len(filtwave)-len(signal)))
    WinWave = np.append(np.zeros(len(filtwave)-len(signal)), WinWave)
    width = (highcut - lowcut) / 2.0
    window = firwin(ntaps, [lowcut, highcut], window=window, width=width,
                    nyq=nyq, pass_zero=False)
    
    # two-pass filtering (time and time-reverse) to remove phase shift
    firstpass = lfilter(window, np.abs(WinWave).max(), WinWave)
    filtwave = lfilter(window, np.abs(firstpass).max(), firstpass[::-1])
    #firstpass = sig_convolve(WinWave, window, mode='full')
    #filtwave = sig_convolve(firstpass[::-1], window, mode='full')[::-1]
    if T0 == 50:
        plot_kaiser_demo(tr, WinWave, filtwave)
    return filtwave

def plot_kaiser_demo(tr, signal, filtered):
    """
    ...
    # plot weight window
    ax2 = ax1.twinx()
    relativetime = np.arange(len(filtered))
    ax2.plot(relativetime, filtered, 'r-', linewidth=0.5)
    ax2.set_title(tr.id)
    f.tight_layout()
    plt.show()

def measure_group_arrival(tr, periods, alpha=FTAN_ALPHA, period=50):
    """
